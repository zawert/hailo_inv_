...
export default function ReadItem({ config, selectedItem }) {
  ...
  useEffect(() => {
    ...
    if (currentResult) {
      const { items, inventory, ...others } = currentResult;

      // When it accesses the /payment/inventory/ endpoint,
      // it receives an inventory.item instead of just item
      // and breaks the code, but now we can check if items exists,
      // and if it doesn't we can access inventory.items and bring
      ...
      if (items) {
        ...
        setItemsList(inventory.items);
        setCurrentErp({ ...inventory.items, ...others, ...inventory });
      }
    }
